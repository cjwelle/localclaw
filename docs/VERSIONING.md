...
- **Tags are `v` + VERSION.** A release tag is `vMAJOR.MINOR.PATCH` (e.g.
  `v0.1.0`). A release job must refuse to publish unless its received tag
  equals `v$(cat VERSION)` **exactly**. For the current project that means the
...
4. **The tag pipeline gates the release.** It re-runs lint, the secret scan, the
   Ubuntu tests, and the macOS tests, then verifies that the received tag is
   `v0.2.0` before building the release artifact and its checksums.
...
- `verify` reads a provider-supplied tag variable when configured, or an
  explicit `--tag`. With a tag present it fails unless the tag is exactly
  `v<VERSION>`; with no tag it only checks the format.
- `prepare` refuses a non-SemVer version and refuses any version that is not
  **strictly greater** than the current one — it never downgrades and never
  auto-increments.
- `LOCALCLAW_RELEASE_ROOT` overrides the repository root the tool operates on. The
