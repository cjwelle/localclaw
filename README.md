...
> configured provider to read Vault unseal shares, and will prompt for the
> note reference at launch if `VAULT_UNSEAL_REF` and any project-scoped
> `<PROJECT>_VAULT_UNSEAL_REF` are blank.
...
7. **Work:** `make work-session` (or `scripts/work-session`) starts a foreground
   ...
   stopped). It unlocks the configured password manager, looks for
   `VAULT_UNSEAL_REF`, then `<PROJECT>_VAULT_UNSEAL_REF` when
   `LOCALCLAW_PROJECT` is set, and prompts for the note reference only if both
   are blank. Validate first with `scripts/work-session --dry-run`.
